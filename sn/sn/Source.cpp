...
void xu_ly_ran(int toadox[], int toadoy[], int x, int y, int& xqua, int& yqua)
{
	...
	them(toadox, x); //1 hàm thêm sl s? t?ng lên 1 d?n ??n b? sai
	them(toadoy, y); // các b?n vào hàm thêm s? th?y sl++

	if (kt_ran_an_qua(xqua, yqua, toadox[0], toadoy[0]) == false)
	{
		//b2: xoa toa do cuoi mang
		xoa(toadox, sl - 1); //hàm xóa c?ng có gi?m sl
		xoa(toadoy, sl - 1); //nên s? bù l?i sl thay ??i ? trên

	}
	else
	{
		//r?n s? t?ng 2 ??t khi ?n qu?. vì lí do hàm thêm ?ã t?ng sl 2 l?n
		// vì v?y nên mình c?n gi?m sl l?i 1 l?n th? là ?úng
		sl--;// ?ây là 1 cách fix c?a mình. cho s? l??ng gi?m l?i nhé
		//--------------------------------
		tao_qua(xqua, yqua, toadox, toadoy);
	}
	//b3: ve ran
